    ...
    var body: some View {
        Button(action: {
            ...
            VStack(alignment:.leading,spacing: 10){
                ...
                .cornerRadius(15)
                .minimumScaleFactor(0.01)
                .accessibilityLabel("Proyek \(project.projectName), Lokasi di \(project.location), tanggal mulai \(dateFormatter.string(from: project.startDate)) sampai \(dateFormatter.string(from: project.endDate)), jam kegiatan \(timeFormatter.string(from: project.startTime)) sampai \(timeFormatter.string(from: project.endTime)), tugas yang terisi enam dari sepuluh tugas") // tugas masih data dummy
        }
        .background(
